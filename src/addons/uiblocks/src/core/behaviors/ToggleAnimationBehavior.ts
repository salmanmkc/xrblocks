 ...
 */
export class ToggleAnimationBehavior extends UICardBehavior<ToggleAnimationConfig> {
  private isAnimating = false;
  private progress = 0; // 0 to 1.
  private targetState: 'visible' | 'hidden' = 'visible';
  ...
  update() {
    if (!this.card || !this.isAnimating) return;
    ...
    // Check completion.
    if (
      (this.targetState === 'visible' && this.progress >= 1) ||

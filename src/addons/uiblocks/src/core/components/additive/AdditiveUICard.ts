 ...
 */
export class AdditiveUICard extends UICard {
  private renderTarget: THREE.WebGLRenderTarget;
  private renderCamera: THREE.OrthographicCamera;
  public targetContainer: uikit.Custom;
  private blendMode: 'screen' | 'additive';
  ...
  private updateTextureResolution() {
    ...
    if (
      this.renderTarget.width !== targetW ||
   ...
   */
  private renderInternal(renderer: THREE.WebGLRenderer) {
    if (!this.renderTarget || !this.targetContainer) return;
    ...
    this.renderCamera.matrixWorld.decompose(
      this.renderCamera.position,
      this.renderCamera.quaternion,
      this.renderCamera.scale
    );
